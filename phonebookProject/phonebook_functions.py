...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okat to exit application?"):
        # This closes app
        self.master.destroy()
        os._exit(0)
...
def count_records(cur):
    ...
    # Call first index of tuple
    count = cur.fetchone()[0]
    # Return cursor so we have access again, and the count
    return cur, count

# Select item in ListBox
def onSelect(self,event):
    # Calling the event is the self.lstsList1 widget
...
def addToList(self):
    ...
    var_fname = var_fname.strip() # Removes any blank spaces before and after user's entry
    var_lname = var_lname.strip()

    var_fname = var_fname.title() # Ensure first character of each word is capitalized
    ...
    if not "@" and not "." in var_email: # Will use this soon
        print("Incorrect email format!")
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_email) > 0): # Make user enter both names
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cur = conn.cursor()
            # Check dB for existance of fullname, if so we will alert user and disregard request
            cur.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_fullname))
            count = cur.fetchone()[0]
            chkName = count
            if chkName == 0: # if count 0, that fullname does not already exist in dB
...
def onDelete(self):
    ...
    with conn:
        cur = conn.cursor()
        # Check count to ensure this is not the last record in dB...will get error if you delete last record
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
...
##  onRefresh(self) # Update listbox with changes
    try:
        index = self.listList1.curselection()[0]
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0):  # Ensure there is data present
        ...
        with conn:
            cur = conn.cursor()
            # Count records to see if the user's changes are already in dB
            ...
            print(count2)
            if count == 0 or count2 == 0:  # Proceed if changes are not already in dB
